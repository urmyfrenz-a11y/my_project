...
const SKIP_RE =
  /선정\s?결과|선정결과|결과\s?공고|결과\s?발표|최종\s?선정|선정기업|선정\s?계획|평가\s?결과|합격|명단|당첨|개정|폐지|규정|훈령|고시|공고문\s?정정|정정\s?공고|낙찰|계약\s?체결|사업\s?종료|폐강|지정\s?신청|기관\s?지정|지정\s?공고|시험평가기관|평가기관|인증기관|위탁기관|수행기관\s?지정|채용|입찰|의견\s?수렴|공청회|설명회|간담회/;
